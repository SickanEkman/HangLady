...
#functions
def show_word(game_d): #function called from word_output()
...
def letter_freq(corpus_d, used_letters):
    ...
    for i in corpus_d: #for every entry in corpus_d
        entry = i * corpus_d.get(i) # entry = the key, 'value' times (e.g. "cowcowcow")
        for letter in entry:
...
while "." in regexp:
    ...
    if feedback == "done":
        ...
        if rightWrong == "yes":
            ...
            game_d = mod_game_d(placement_l, most_common_letter) #adds new knowned letters to game_d
        elif rightWrong == "no":
